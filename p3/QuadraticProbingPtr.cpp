//Author: Kevin Nguyen, Haowen(Angela) Zhou
	...
	QuadraticPtrHashTable<HashedObj>::~QuadraticPtrHashTable()
 	   ...
 	   for (int i =0; i<array.size();i++)
           {
 	     if (array[i]!=NULL&&array[i]!=&ITEM_NOT_FOUND)
             {
             	delete array[i];
	     }
           }
        ...
        {
 	    int* insertNum =new int;
            *insertNum=x;
                // Insert x as active
            int currentPos = findPos( x );
            if( isActive(array, currentPos ) )
                return;
            array[ currentPos ] = insertNum;
...
/* 7*/      return currentPos;
        ...
        {
            int currentPos = findPos( x );
            return isActive(array, currentPos ) ? *array[ currentPos ] :& ITEM_NOT_FOUND;
        }
         ...
         */
        template <class HashedObj>
        bool QuadraticPtrHashTable<HashedObj>::isActive(vector<const HashedObj*> &array,int currentPos ) const
        {
